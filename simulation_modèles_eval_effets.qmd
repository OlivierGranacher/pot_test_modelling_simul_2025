...
### Modéle lm avec effets période et groupe sans interaction avec variabilité

```{r}
#| label: modele_lm_periode_groupe_with_variability
#| include: true
# modèle linéaire avec effets de période et de groupe sans interaction avec variabilité
model_lm_ss_inter_var <- lm(rf ~ period + group, data = data_var)
sjPlot::tab_model(model_lm_ss_inter_var, 
                show.ci = FALSE, 
                digits = 3,
                show.se = TRUE, 
                show.stat = TRUE, 
                show.p = TRUE,
                pred.labels = c("(Intercept)", "Période après intervention", "Groupe test"),
                dv.labels = "Modèle linéaire sans interaction avec variabilité")
# performance du modèle
# performance::check_model(model_lm_ss_inter_var)
# plot model results
sjPlot::plot_model(model_lm_ss_inter_var, 
                show.values = TRUE, 
                show.p = TRUE, 
                value.offset = .3,
                title = "Modèle linéaire sans interaction avec variabilité",
                ci.lvl = 0.95
) +
coord_cartesian(ylim = c(0, 0.05)) 
```

### Modéle lm avec effets période et groupe avec interaction avec variabilité

\`\`\`{r} #\| label: modele_lm_periode_groupe_interaction_with_variability #\| include:
true \# modèle linéaire avec effets de période et de groupe avec interaction avec
variabilité model_lm_avec_inter_var \<- lm(rf \~ period \* group, data = data_var)
sjPlot::tab_model(model_lm_avec_inter_var, show.ci = FALSE, digits = 4, show.se = TRUE,
show.stat = TRUE, show.p = TRUE, pred.labels = c("(Intercept)", "Période après
intervention", "Groupe test", "Interaction période: groupe test"), dv.labels = "Modèle
linéaire avec interaction avec variabilité") \# performance du modèle \#
performance::check_model(model_lm_avec_inter_var) \# plot model results
sjPlot::plot_model(model_lm_avec_inter_var, show.values = TRUE, show.p = TRUE,
value.offset = .3, title = "Modèle linéaire avec interaction avec variabilité", ci.lvl =
0.95 )